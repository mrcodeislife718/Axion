...
export class AxionKeyRing {
  ...
  rotate({ reason = 'rotation' } = {}) {
    ...
    for (const key of meta.keys) if (key.status === 'active') { key.status = 'superseded'; key.supersededAt = now(); }
  ...
  revoke(keyId, { reason = 'revoked' } = {}) {
    ...
    key.status = 'revoked'; key.revokedAt = now(); key.revocationReason = reason;
    this.save(meta);
    return key;
  }
  sign(payload) {
    ...
    return { keyId: key.keyId, algorithm: 'Ed25519', payloadDigest, signature: crypto.sign(null, Buffer.from(payloadDigest), privatePem).toString('base64url'), signedAt: now() };
  }
  verify(payload, envelope, { allowSuperseded = true } = {}) {
    if (!envelope?.keyId || !envelope.signature || !envelope.payloadDigest) return { verified: false, reason: 'invalid-signature-envelope' };
    const key = this.metadata().keys.find(item => item.keyId === envelope.keyId);
    if (!key) return { verified: false, reason: 'unknown-key' };
    if (key.status === 'revoked') return { verified: false, reason: 'key-revoked' };
    if (!allowSuperseded && key.status !== 'active') return { verified: false, reason: 'key-not-active' };
    const calculated = digest(payload);
    if (calculated !== envelope.payloadDigest) return { verified: false, reason: 'payload-digest-mismatch' };
    return { verified: crypto.verify(null, Buffer.from(calculated), key.publicKeyPem, Buffer.from(envelope.signature, 'base64url')), keyId: key.keyId, keyStatus: key.status };
  }
  publicKeys() { return this.metadata().keys.map(key => ({ ...key })); }
}
